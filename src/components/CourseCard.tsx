...
import { Clock, BarChart3, Users, Play, Calendar, Video, Wifi } from "lucide-react";
...
const CourseCard = ({
  ...
  const translatedTitle = t(`courseData:${id}.title`, { defaultValue: title });
  const translatedDescription = t(`courseData:${id}.description`, { defaultValue: description });
  ...
  return (
    <div className="group relative bg-card rounded-xl overflow-hidden border border-border hover:border-secondary/50 transition-all duration-300 hover:shadow-cyan hover:-translate-y-1">
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <Badge className={cn("absolute top-3 left-3 flex items-center gap-1.5 font-bold text-xs", formatInfo.className)}>
        ...
        <div className="absolute top-3 right-3">
          {isFree ? (
            ...
            <Badge className="bg-card/90 backdrop-blur-sm text-card-foreground font-bold text-sm px-3 py-1 border border-border">
              {originalPrice && (
                <span className="line-through text-muted-foreground mr-1.5 text-xs">€{originalPrice}</span>
              )}
              €{price}
            </Badge>
          )}
      ...
      <div className="p-5 flex flex-col">
        ...
        {/* Description */}
        <p className="text-muted-foreground text-sm mb-3 line-clamp-2 h-10">{translatedDescription}</p>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-3 text-xs">
          {format === 'live' && countdown && (
            <div className="flex items-center gap-1.5 text-secondary">
              ...
              <span className="font-medium">{t('card.nextIn', { defaultValue: 'Next in' })} {countdown}</span>
            </div>
          )}
          ...
          {format === 'recorded' && (
            <div className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <span>{t('card.onDemand', { defaultValue: 'On-demand' })}</span>
            </div>
          )}
        ...
        <div className="flex items-center mb-4">
          <Badge variant="outline" className={cn("text-xs", levelColors[level])}>
            ...
            {t(`card.levels.${level.toLowerCase()}`, { defaultValue: level })}
          </Badge>
        </div>
        ...
        <Link to={`/courses/${id}`}>
          ...
          >
            {isFree 
              ? t('card.startFree', { defaultValue: 'Start Free' })
              : t('card.enrollFor', { price, defaultValue: `Enroll for €${price}` })
            }
          </Button>
        </Link>
      </div>
