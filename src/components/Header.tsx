      ...
      <nav className="container mx-auto px-4 py-5">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2 group">
            <img alt="CodeSkillz" src="/CodeSkillz/icon.webp" className="w-8 h-8 text-secondary transition-transform group-hover:scale-125" />
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                <span
                  className={`absolute -bottom-1 left-0 h-0.5 bg-secondary transition-all ${isActive(link.path) ? "w-full" : "w-0 group-hover:w-full"
                    }`}
                />
              </Link>
            ))}
            ...
            <div className="flex flex-col gap-4">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`font-medium transition-colors py-2 ${isActive(link.path)
                      ? "text-secondary"
                      : "text-primary-foreground hover:text-secondary"
                    }`}
                >
