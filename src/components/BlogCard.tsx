...
const BlogCard = ({ post }: BlogCardProps) => {
    ...
    const translatedTitle = t(`blogData:${post.id}.title`, {
        defaultValue: post.title,
    });
    const translatedExcerpt = t(`blogData:${post.id}.excerpt`, {
        defaultValue: post.excerpt,
    });
    const translatedCategory = t(`blogData:${post.id}.category`, {
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-border/50">
                        <div className="flex gap-1 overflow-hidden">
                            {post.tags.slice(0, 2).map((tag) => (
                                <span
                                    key={tag}
                                    className="text-xs text-secondary/80 bg-secondary/10 px-2 py-0.5 rounded-full truncate max-w-[70px]"
                                >
                                    {tag}
                                </span>
                            ))}
                        ...
                        <span
                            className={`text-secondary font-semibold text-sm transition-transform inline-flex items-center gap-1 whitespace-nowrap ${isRTL ? 'group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`}
                        >
                            {t('readMore')}{' '}
                            <span
                                className={
                                    isRTL ? 'rotate-180 inline-block' : ''
                                }
                            >
                                →
                            </span>
