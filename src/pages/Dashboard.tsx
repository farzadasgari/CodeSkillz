    ...
    const menuItems = [
        {
            ...
            shortLabel: t('menu.courses').split(' ')[0],
            icon: BookOpen,
        },
        {
            id: 'classes' as Section,
            label: t('menu.classes'),
            shortLabel: t('menu.classes').split(' ')[0],
            icon: Calendar,
        },
        {
            id: 'progress' as Section,
            label: t('menu.progress'),
            shortLabel: t('menu.progress').split(' ')[0],
            icon: TrendingUp,
        },
        {
            id: 'payments' as Section,
            label: t('menu.payments'),
            shortLabel: t('menu.payments').split(' ')[0],
            icon: CreditCard,
        },
        {
            id: 'achievements' as Section,
            label: t('menu.achievements'),
            shortLabel: t('menu.achievements').split(' ')[0],
            icon: Trophy,
        },
        {
            id: 'settings' as Section,
            label: t('menu.settings'),
            shortLabel: t('menu.settings').split(' ')[0],
            icon: Settings,
        },
        {
            id: 'support' as Section,
            label: t('menu.support'),
            shortLabel: t('menu.support'),
            icon: HelpCircle,
        },
    ];
                        ...
                        <nav className="bg-card rounded-lg border border-border shadow-lg overflow-hidden sticky top-24">
                            {menuItems.map((item) => (
                                ...
                                >
                                    ...
                                    <item.icon
                                        className={`w-5 h-5 flex-shrink-0 ${activeSection === item.id ? '' : 'group-hover:scale-110 transition-transform'}`}
                                    />
